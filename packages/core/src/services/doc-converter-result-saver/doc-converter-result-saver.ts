...
import { IAsset, IConfig, IDocConverterResult } from "@mtfm/core-models";
import { join } from "node:path";
import { to } from "await-to-js";

import { IDocConverterResultSaver } from "./i-doc-converter-result-saver.js";

export class DocConverterResultSaver implements IDocConverterResultSaver {
    ...
    public async execute(docConverterResult: IDocConverterResult): Promise<Result<void, Error[]>> {
        const errors: Error[] = [];

        const saveDocumentResult = await this.__saveDocument(this.__outputPath, docConverterResult.document);

        if (saveDocumentResult.isErr())
            errors.push(saveDocumentResult.error);

        const saveImagesResult = await this.__saveImages(this.__outputPath, docConverterResult.images, this.config);
