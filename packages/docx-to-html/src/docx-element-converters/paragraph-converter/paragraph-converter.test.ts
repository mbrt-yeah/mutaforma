import { CoParagraph, CoTextRun, Doc, DocxNumberingLvl, DocxNumberingSchema } from "@mtfm/core-models";
...
doc.numberings["5"] = new DocxNumberingSchema({
    id: "5",
    idAbstract: "5",
    levels: [
...
describe(`${ParagraphConverter.name}`, () => {
    ...
    describe("#execute", () => {
        ...
        it(`should convert w:p element with properties to ${CoParagraph.name} which contains all extracted properties`, async () => {
            ...
            expect(result.mapping?.element.attrs).toEqual(undefined);
